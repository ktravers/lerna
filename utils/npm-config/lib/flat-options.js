...
/*
 * woo copypaste of https://github.com/npm/cli/blob/2a80c67e/lib/utils/flat-options.js
 * (with auto-formatting and LOTS of manual edits, lol)
 */
...
const buildOmitList = (obj) => {
  ...
  const omit = new Set((obj.omit || []).filter((type) => !include.includes(type)));
  const only = obj.only;
...
const flatten = (obj) => ({
  ...
  packageLock: !(obj["package-lock"] === false || obj.shrinkwrap === false),
  ...
  saveType:
    obj["save-optional"] && obj["save-peer"]
      ? "peerOptional"
      : obj["save-optional"]
      ? "optional"
      : obj["save-dev"]
      ...
      ? "prod"
      : null,
  savePrefix: obj["save-exact"] ? "" : obj["save-prefix"],
...
function getScopesAndAuths(obj) {
